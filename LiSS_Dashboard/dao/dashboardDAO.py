...
class DashboardDAO:
    ...
    def getTopUsers(self):
        cursor = self.conn.cursor()
        query = "SELECT num, userName FROM (SELECT userID, num FROM(((SELECT userID, num FROM(" \
                ...
                ") as W NATURAL INNER JOIN users GROUP BY userID, userName, num ORDER BY num LIMIT 10"
        cursor.execute(query)
        result = []
        for row in cursor:
            result.append(row)
